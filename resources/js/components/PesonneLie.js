...
export default class PersonneLie extends React.Component {
    ...
    componentDidMount(){
        setTimeout(() => {
            document.getElementById("validerP").setAttribute("disabled","disabled");
    ...
    handleChange(event){
        ...
        getData=async()=>{
            if(this.props.num) {
              try {
                  const data=await axios.get("personnelie/getpersonnelie/"+this.props.num)
                  if(data.data){
       ...
       getPersonneLieDetails=(id)=>{
        axios.post('/get/getindividual/personnelie/details',{
            pesonneLieId:id
        }).then((response)=>{
       ...
       <form method="POST">
                   ...
                   <div className="col-md-4">
                    <table>
                     ...
                     <td>
                         <input
                              className={`form-control ${this.state.error_list.personneLieName && "is-invalid"}`}
                             id="nomheritier"
                             style={
                                 inputColor
                             } 
                    ...
                    <table className="table table-dark" style={tableBorder}>
                                            ...
                                            {this.state.fetchData &&   this.state.fetchData.data.map((res,index)=>{
                                                     return(
                                                        ...
                                                        <td style={tableBorder}>{res.nameP}</td>
                                                        <td style={tableBorder}>{res.lieuP}</td>
                                                        <td style={tableBorder}>{res.dateP}</td>
                                                        <td style={tableBorder}>{res.degreP}</td>
                                                        
                                                        <td style={tableBorder}><div class="btn-group" role="group" aria-label="Basic example">
                                                         <button style={{fontSize:"10px"}} type="button" className="btn btn-primary" data-toggle="modal" data-target="#modal-personnelie" onClick={()=> {this.getPersonneLieDetails(res.idPersonneLie)}}>Modifier  <i className="fas fa-edit"></i> </button>
                                                        <button style={{fontSize:"10px"}} type="button" className="btn btn-danger"  data-toggle="modal" data-target="#modal-delete-personnelie" onClick={()=>{ this.deletePersonneLie(res.idPersonneLie) }}>Supprimer   <i className="fas fa-trash"></i> </button>
                                                         {/* <button type="button" class="btn btn-primary">Right</button> */}
                                                       </div></td> 
                                                       <UpdatePersonneLie  modalId={this.state.idPersonneLie} personneLieData={this.state}  />
                                                   
                                                       <DeletePersonneLie  personneLieId={this.state.idToDelete} />
                                                   </tr>
                                                  )
                                                  })}
